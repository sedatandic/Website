...
const offices = [
  ...
  { name: 'Doha', type: 'trading', left: 64.6, top: 43 },
  { name: 'Dubai', type: 'trading', left: 66.2, top: 43.5 },
...
const Marker = ({ name, type, left, top, index }) => {
  const color = COLORS[type];
  return (
    <div
      className="absolute -translate-x-1/2 -translate-y-1/2 group"
      style={{ left: `${left}%`, top: `${top}%` }}
      data-testid={`office-marker-${name.toLowerCase()}`}
    >
      {/* pulsing ring */}
      <motion.span
        className="absolute rounded-full"
        style={{ width: 16, height: 16, left: -8, top: -8, background: color }}
      ...
      {/* solid dot */}
      <span
        className="relative block rounded-full ring-2 ring-white"
        style={{ width: 9, height: 9, background: color }}
      />
      {/* label on hover */}
      <span
        className="absolute left-1/2 -translate-x-1/2 -top-6 whitespace-nowrap px-2 py-0.5 rounded text-[10px] font-semibold text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none"
...
export const AnimatedOfficeMap = () => (
  <div className="mb-6" data-testid="animated-office-map">
    ...
    <div className="relative w-full rounded-xl border overflow-hidden" style={{ borderColor: '#e5e7eb', background: '#fbfbfc', aspectRatio: '1264 / 848' }}>
      <img src={MAP_URL} alt="World map of Peninsula Agritrade offices" className="absolute inset-0 w-full h-full object-contain" style={{ opacity: 0.55 }} />
      {offices.map((o, i) => (
